...
export class RecordService {
  ...
  async findDetailByRecordDto(id: string, includeRelations = false) {
    const query = this.recordDetailRepo
      ...
      .leftJoinAndSelect("detail_x_record.politic_party", "politic_party")
      .leftJoinAndSelect(
    ...
    const detailDto: RecordDetailDto[] = res.map((item) => {
      return {
        ...
        votes: item.votes || 0,
        politic_party: {
          politic_party_id: item?.politic_party?.id || "",
          politic_party_name: item?.politic_party?.name || "",
          politic_party_code: item?.politic_party?.code || "",
          politic_party_image: item?.politic_party?.image_url || "",
        },
        political_alliance: {
  ...
  async getCongressResult(
    ...
    const partyPositionsWithExtra: QuotientPartyDto[] = partyPositions?.map(
      (item: QuotientPartyDto) => {
        ...
        if (
          extraPosition.some(
            (d) => d.politic_party_id === item?.politic_party_id
          )
        )
          extra = 1;
        return {
  ...
  async getMayorResult(
    ...
    const partyMark: PartyMayorMarkDto[] = partyMarks?.map((item: any) => {
      return {
        politic_party_id: item.politic_party_id || "",
        politic_party_code: item.code || "",
        politic_party_image: item.image_url || "",
    ...
    while (index < positions) {
      ...
      const newPosition: PartyMayorPositionDto = {
        politic_party_id: sortArray[0]?.politic_party_id || "",
        politic_party_code: sortArray[0]?.politic_party_code || "",
        politic_party_image: sortArray[0]?.politic_party_image || "",
        position: index,
      };
  ...
  async create(
    ...
    const idsParty =
      input?.detail
        ?.filter((item) => item.politic_party_id)
        .map((item) => item.politic_party_id) || [];
    const idsAlliance =
      input?.detail
    ...
    if (input.detail.length > 0) {
      for (const detail of input.detail) {
        const party = parties.find((i) => i.id === detail.politic_party_id);
        ...
        if (party) newDetail.politic_party = party;
  ...
  private async mergeDetail(
    ...
    detailInput.forEach((detail) => {
      ...
      if (indexDetail < 0) {
        ...
        if (party) recordDetailDB[indexDetail].politic_party = party;
