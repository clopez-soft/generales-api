...
import { padLeft } from "src/helper/util";
...
export class SeederService {
  constructor(
    ...
    private readonly PoliticPartyRepository: Repository<PoliticPartyEntity>,
  ...
  async createPoliticParty() {
    console.info("--- creating politic party ---");

    const politicParties = getSeedPoliticParty();
    for (const politicParty of politicParties) {
      try {
        const dbParty = await this.PoliticPartyRepository.findOne({
        ...
        const newPoliticParty =
          this.PoliticPartyRepository.create(politicParty);
        const saved = await this.PoliticPartyRepository.save(newPoliticParty);
        console.log(
  ...
  async createAlliances() {
    ...
    if (politicAlliances.length === 0) return;
    ...
    for (const ballot of ballots) {
      try {
        ...
        for (const detail of ballot.detail) {
          ...
          if (detail.party) {
            const party = await this.PoliticPartyRepository.findOne({
              where: { code: detail.party },
            });
            if (!party) {
              console.log(
                `%c party : ${detail.party} not exist`,
                "background:orange; color:white"
              );
              continue;
            }

            newDetail.politic_party = party;
          }
  ...
  async createCandidatesPresident() {
    ...
    for (const candidate of candidates) {
      try {
        ...
        if (candidate.party) {
          const party = await this.PoliticPartyRepository.findOne({
            where: { code: candidate.party },
          });
          if (!party) {
            console.log(
              `%c party : ${candidate.party} not exist`,
              "background:orange; color:white"
            );
            continue;
          }
          newCandidate.politic_party = party;
        }
  ...
  async createCandidatesCongress() {
    ...
    for (const candidate of candidates) {
      try {
        ...
        const tempNew = {
          ...
          box_number: candidate.box_number,
          level: candidate.level,
          image_url: `https://electoral-general-yoro.sfo3.digitaloceanspaces.com/Candidatos/Diputado/${padLeft(candidate.box_number)}.png`,
    ...
    //       where: { ballot: ballot },
    //       relations: ["politic_party", "political_alliance"],
    //     });
    //     console.log(
    ...
    //             level: ballot.level,
    //             department: ballot.department,
    //             politic_party: detail.politic_party,
  ...
  async createCandidatesMayor() {
    ...
    for (const candidate of candidates) {
      try {
        ...
        if (dbCandidate) {
          console.log(
            `%c candidate : ${candidate.level} - ${candidate.box_number} - ${candidate.municipality_code} already exist`,
            "background:orange; color:white"
          );
          continue;
        }
